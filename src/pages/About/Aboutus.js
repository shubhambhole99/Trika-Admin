...
export default () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`${baseurl}/production/api/createabout`, pageData, {});
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`${baseurl}/api/deleteMedia/${id}`);
      console.log(response);
  ...
  useEffect(() => {
    axios.get(`${baseurl}/api/get/about`)
      .then(response => {
