...
export default () => {
  ...
  const handleUpload = () => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        pageData.append('carouselImages', getPredefinedUrl(key));
        pageData.append('isActive', isActive);
        const body={
          name,
          carouselImages:getPredefinedUrl(key),
          isActive
        }
        console.log(body)
        // Example: Posting additional form data using Axios
        const responseFormData = await axios.post(`${baseurl}/api/addimage`, body, {
          headers: {
            Authorization: `${token}`,
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`${baseurl}/api/deleteimage/${id}`, {
      headers: {
        Authorization: token
      }
    })
  ...
  useEffect(() => {
    axios.get(`${baseurl}/api/getimage`)
