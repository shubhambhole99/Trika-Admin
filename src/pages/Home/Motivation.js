...
export default () => {
 ...
 const handleUpload = () => {
   ...
   reader.onload = async (event) => {
     ...
     try {
      ...
      isActive,
       ...
       const responseFormData = await axios.post(`${baseurl}/api/create/motivation`, body, {
         headers: {
           Authorization: `${token}`,
  ...
  useEffect(() => {
    axios.get(`${baseurl}/api/get/motivations?page=${currentPage}&perPage=${itemsPerPage}`)
      .then(response => {
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`${baseurl}/api/delete/motivation/${id}`, {
      headers: {
        Authorization: `${token}`
      }
    })
  ...
  const handleEditModal = (item) => {
    ...
    setEditAttribbute(item.Attribbute);
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const response = await axios.put(`${baseurl}/api/update/motivation/${editItemId}`, editData, {
        headers: {
          Authorization: `${token}`
        }
      });
                  ...
                  <Col xs={12} lg={12}>
                    <Card border="light" className="shadow-sm">
                      ...
                      <Modal show={showModal && editMode} onHide={() => setEditMode(false)}>
                        ...
                        <Modal.Body>
                          <Form>
                            <Form.Group className="mb-3" controlId="editAttribbute">
                              <Form.Label>Attribbute</Form.Label>
