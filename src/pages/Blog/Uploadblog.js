...
export default () => {
    ...
    const handleUpload = () => {
      ...
      reader.onload = async (event) => {
        ...
        try {
          ...
          const responseFormData = await axios.post(`${baseurl}/api/createblog`, body, {
            headers: {
              Authorization: `${token}`,
    ...
    useEffect(() => {
        axios.get(`${baseurl}api/blog?page=${currentPage}&perPage=${itemsPerPage}`)
            .then(response => {
    ...
    const handleDelete = (id) => {
        const token = localStorage.getItem('token');
        axios.delete(`${baseurl}/api/blog/${id}`, {
            headers: {
                Authorization: `${token}`
            }
        })
    ...
    const handleEditSubmit = async () => {
        ...
        try {
            const response = await axios.put(`${baseurl}/api/blog/${editItemId}`, editData, {
                headers: {
                    Authorization: `${token}`
                }
            });
